...
def get_rl_algo(env):
    policy_kwargs = dict(
        ...
        simba_hidden_dim=SIMBA_HIDDEN_DIM,
        simba_num_blocks=SIMBA_NUM_BLOCKS,
    )
...
def train():
    ...
    # Monkey-patch to fix device mismatch issue in imitation library
    # Patch the policy's evaluate_actions method to ensure actions are moved to the correct device
    original_evaluate_actions = algo_ppo.policy.evaluate_actions
    def patched_evaluate_actions(obs, actions):
        # Move actions to the policy's device if they're not already there
        policy_device = next(algo_ppo.policy.parameters()).device
        if isinstance(actions, th.Tensor) and actions.device != policy_device:
            actions = actions.to(device=policy_device)
        # Handle dict observations if needed
        if isinstance(obs, dict):
            obs = {k: v.to(device=policy_device) if isinstance(v, th.Tensor) and v.device != policy_device else v 
                   for k, v in obs.items()}
        elif isinstance(obs, th.Tensor) and obs.device != policy_device:
            obs = obs.to(device=policy_device)
        return original_evaluate_actions(obs, actions)
    algo_ppo.policy.evaluate_actions = patched_evaluate_actions
...
if __name__ == "__main__":
    ...
    # python examples/train_with_bc_simba.py --config-file-name configs/train/flycraft_annealing.json

    parser = argparse.ArgumentParser(description="传入配置文件")
    parser.add_argument("--config-file-name", type=str, help="配置文件名", default="configs/train/flycraft_annealing.json")
    ...
    # Simba configs
    SIMBA_HIDDEN_DIM = train_config["rl_bc"].get("simba_hidden_dim", 256)
    SIMBA_NUM_BLOCKS = train_config["rl_bc"].get("simba_num_blocks", 2)
    ...
    # Use SimBaActorCriticPolicy for loading
    algo_ppo = PPOWithBCLoss.load(str((policy_save_dir / POLICY_FILE_SAVE_NAME).absolute()), policy=SimBaActorCriticPolicy)
