...
from utils_my.sb3.my_schedule import linear_schedule, linear_decay_schedule
...
def train():
    ...
    # Load anchor model for KL loss
    algo_ppo_for_kl_loss = PPOWithBCLoss.load(str((bc_policy_save_dir / BC_POLICY_FILE_NAME).absolute()), policy=SimBaActorCriticPolicy)
    algo_ppo_for_kl_loss.policy.set_training_mode(False)
    for param in algo_ppo_for_kl_loss.policy.parameters():
        param.requires_grad = False
    
    # Annealing schedule
    if KL_ANNEALING:
        kl_coef = linear_decay_schedule(KL_ANNEALING_START, KL_ANNEALING_END)
    else:
        kl_coef = KL_WITH_BC_MODEL_COEF
...
if __name__ == "__main__":
    ...
    # python examples/train_with_rl_bc_simba.py --config_file_name configs/train/flycraft_annealing.json
    ...
    BC_POLICY_AFTER_VALUE_HEAD_TRAINED_FILE_NAME = train_config["bc"].get("policy_after_value_head_trained_file_save_name", "policy_after_value_head_trained.zip")
    ...
    KL_ANNEALING_START = train_config["rl_bc"].get("kl_annealing_start", 0.2)
    KL_ANNEALING_END = train_config["rl_bc"].get("kl_annealing_end", 0.01)
