...
class PPOWithBCLoss(PPO):
    ...
    def __init__(self,
        ...
        if policy_kwargs and "use_sde" in policy_kwargs:
            policy_kwargs.pop("use_sde")
        ...
        if self.bc_trained_algo is not None:
            logging.getLogger(__name__).info("Successfully load the bc trained model.")
            self.bc_trained_algo.policy.set_training_mode(False)
            for param in self.bc_trained_algo.policy.parameters():
                param.requires_grad = False

    def _setup_model(self) -> None:
        if self.policy_kwargs and "use_sde" in self.policy_kwargs:
    ...
    def train(self) -> None:
        ...
        self.logger.record("train/kl_coef", tmp_kl_coef)
