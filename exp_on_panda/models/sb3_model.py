...
PROJECTT_ROOT_DIR = Path(__file__).parent.parent.parent.parent
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                # track kl coef for logging
                if isinstance(tmp_kl_coef, th.Tensor):
                    kl_coefs.append(tmp_kl_coef.item())
                else:
                    kl_coefs.append(float(tmp_kl_coef))
        ...
        if len(kl_coefs) > 0:
            self.logger.record("train/current_kl_coef", np.mean(kl_coefs))
