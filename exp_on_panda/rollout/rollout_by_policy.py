...
def rollout_by_goal_with_policy(env: ScaledObservationWrapper, goal: np.ndarray, policy: policies.ActorCriticPolicy, progress_info: str = None):
    """_summary_
    ...
    while(not (terminated or truncated)):
        ...
        if terminated or truncated:
            ...
            if terminated:
                print(f"\033[32m success. goal: ({goal[0]}, {goal[1]}, {goal[2]}), steps: {step_cnt}{progress_str} \033[0m")
            else:
                print(f"\033[31m truncated. goal: ({goal[0]}, {goal[1]}, {goal[2]}), steps: {step_cnt}{progress_str} \033[0m")
            break
