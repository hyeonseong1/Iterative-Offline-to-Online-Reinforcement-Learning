...
register_my_env(goal_range=0.3, distance_threshold=0.01, max_episode_steps=100)  # Note: max_episode_steps, modify this value according to the environment file configuration!!!!
...
def evaluate(
    ...
    print(f"env_id used in testing: {env_id}, use nmr: {wrap_env_with_nmr}, nmr waypoint: {nmr_waypoint}, nmr waypoint delta: {nmr_waypoint_delta}, nmr jump: {nmr_jump_cnt}")
...
# waypoint [0.0, 0.0, 0.0] is the same as the original environment, test the correctness of waypoint wrapper
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config-file-name", type=str, help="Configuration file name", default="configs/pointmaze-diverse-g-dense/seed5/sparse_256_256_lambda_1e-1.json")
    parser.add_argument("--test-algo", type=str, help="Algorithm to test, rl, rl_bc, bc, random", default="rl")
    parser.add_argument("--eval-process-num", type=int, help="Number of processes used for testing", default=1)
    parser.add_argument("--eval-episode-num", type=int, help="Number of episodes to test", default=1)
    parser.add_argument("--env-id", type=str, help="ID of the env used for testing", default="none")
    parser.add_argument("--wrap-env-with-nmr", action="store_true", help="Whether the environment is set to non-Markovian reward")
    parser.add_argument("--nmr-waypoint", action="store_true", help="Whether to use waypoint type NMR")
    parser.add_argument("--nmr-waypoint-delta", type=float, nargs='+', default=[0.0, 0.0, 0.1], help="waypoint_delta")
    parser.add_argument("--nmr-jump", action="store_true", help="Whether to use jump type NMR")
    parser.add_argument("--nmr-jump-cnt", type=int, default=2, help="Number of goals for NMR Jump")
    parser.add_argument("--nmr-length", type=int, default=10, help="Length of NMR")
    parser.add_argument("--use-original-reward", action="store_true", help="Whether to use the original reward of the environment")
    parser.add_argument("--non-terminal-reward", type=float, default=0.0)
    parser.add_argument("--terminal-reward", type=float, default=1.0)
    parser.add_argument("--seed", type=int, help="Seed used for testing", default=10)
